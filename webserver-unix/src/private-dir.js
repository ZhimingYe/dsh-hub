...
export function ensurePrivateDirectory(dir) {
  ...
  if ((info.mode & 0o077) !== 0) {
    ...
    if ((lstatSync(dir).mode & 0o077) !== 0) {
      throw new Error(`unix socket dir must be mode 0700: ${dir}`)
    }
  }
}
...
function assertPrivateDirectory(dir) {
  ...
  if (info.isSymbolicLink()) {
    throw new Error(`unix socket dir is a symlink: ${dir}`)
  }
  if (!info.isDirectory()) {
    throw new Error(`unix socket dir is not a directory: ${dir}`)
  }
  const uid = process.getuid?.()
  if (uid !== undefined && info.uid !== uid) {
    throw new Error(`unix socket dir is not owned by this user: ${dir}`)
  }
}
